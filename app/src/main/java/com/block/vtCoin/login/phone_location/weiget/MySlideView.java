package com.block.vtCoin.login.phone_location.weiget;
...
public class MySlideView extends View {
    ...
    private void initView() {
        ...
        mBound = new Rect();
        backgroundColor = getResources().getColor(R.color.white0);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < TelephoneLocationActivity.pinyinList.size(); i++) {
            String textView = TelephoneLocationActivity.pinyinList.get(i);
            if (i == position - 1) {
                paint.setColor(getResources().getColor(R.color.red));
                selectTxt = TelephoneLocationActivity.pinyinList.get(i);
            ...
            mTextHeight += mHeight / TelephoneLocationActivity.pinyinList.size();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                backgroundColor = getResources().getColor(R.color.white0);
                mTextHeight = mHeight / TelephoneLocationActivity.pinyinList.size();
                position = y / (mHeight / (TelephoneLocationActivity.pinyinList.size() + 1));
                invalidate();
                break;
            case MotionEvent.ACTION_MOVE:
                if (isSlide) {
                    backgroundColor = getResources().getColor(R.color.white0);
                    mTextHeight = mHeight / TelephoneLocationActivity.pinyinList.size();
                    position = y / (mHeight / TelephoneLocationActivity.pinyinList.size() + 1) + 1;
                    invalidate();
                }
                break;
            case MotionEvent.ACTION_UP:
